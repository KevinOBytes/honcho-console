...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(await client.listSessions(workspace, page, size));
  } catch (error) {
    const status = error instanceof Error && /Invalid (pagination|workspace)/.test(error.message) ? 400 : 502;
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to load sessions." }, { status });
  }
}
